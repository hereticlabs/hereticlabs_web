...
( function ( $ ) {
	...
	function slider_loaded() {
		setTimeout( function () {
			$( '.txtanim1 span' ).each( function ( i ) {
				setTimeout( function () {
					$( '.txtanim1 span' ).eq( i ).css( { "transform": "translate3d(0px, 0%, 0px)" } );
	...
	if ( ! isMobile ) {
		...
		txtanim1Span.css( { "display": "inline-block", "transform": "translate3d(0px, 100%, 0px)", "transition": "all 0.3s ease 0s" } );
		...
		$( '.txt1-wrap' ).each( function () {
			if ( isScrolledIntoView( this ) === true ) {
				...
				$this.each( function ( i ) {
					setTimeout( function () {
						$this.eq( i ).css( { "transform": "translate3d(0px, 0%, 0px)", "opacity": "1" } );
	...
	function bgParallax() {
		if ( $( ".parallax" ).length ) {
			$( ".parallax" ).each( function () {
				...
				$( this ).css( { backgroundImage: "url(" + img + ")", backgroundPosition: "50%" + "bottom" } );
    ...
    9. Magnific Popup
    ==================================*/
	$( '.expand-img' ).magnificPopup( { type: 'image', gallery: { enabled: true } } );

	$( '.expand-video' ).magnificPopup( { type: 'iframe', gallery: { enabled: true } } );

	/*================================
    10. slicknav Mobile Menu
    ==================================*/
	$( 'ul#nav_menu' ).slicknav( { prependTo: ".responsive-menu-wrap" } );
	...
	function smoothScrolling( $links, $topGap ) {
		...
		links.on( "click", function () {
			if ( location.pathname.replace( /^\//, '' ) === this.pathname.replace( /^\//, '' ) && location.hostname === this.hostname ) {
				...
				if ( target.length ) {
					$( "html, body" ).animate( { scrollTop: target.offset().top - topGap }, 1000, "easeInOutExpo" );
					return false;
				}
			}
			return false;
    ...
    13. Gallery Masonary
    ==================================*/
	$( '#container' ).imagesLoaded( function () { // filter items on button click
		...
		// init Isotope
		var $grid = $( '.portfolio-masonary' ).isotope( { itemSelector: '.prt-grid', percentPosition: true, masonry: { // use outer width of grid-sizer for columnWidth
	...
	$( 'form#cf button#cnt_submit' ).on( 'click', function () {
		...
		if ( name != '' && email != '' && msg != '' ) {
			...
			$.ajax( { type: "POST", url: "mail.php", data: values, success: function () {
					$( '#name' ).val( '' );
					$( '#email' ).val( '' );
					$( '#msg' ).val( '' );
